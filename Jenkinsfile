  ...
  timeout(60) {
    ansiColor('xterm') {
      try {
        ...
        withEnv(['COLCON_BUILD_EXTRA_ARGS=--cmake-args -DBUILD_TESTS=ON --packages-select maliput multilane maliput-utilities dragway maliput-integration-test',
                 'COLCON_TEST_EXTRA_ARGS=--packages-select maliput multilane maliput-utilities dragway maliput-integration-test']) {
