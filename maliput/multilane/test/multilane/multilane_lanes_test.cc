...
GTEST_TEST(MultilaneLanesTest, Rot3) {
  ...
  EXPECT_TRUE(CompareMatrices(rpy90.apply({1., 0., 0.}),
                              drake::Vector3<double>(0., 0., -1.), kVeryExact));
  EXPECT_TRUE(CompareMatrices(rpy90.apply({0., 1., 0.}),
                              drake::Vector3<double>(0., 1., 0.), kVeryExact));
  EXPECT_TRUE(CompareMatrices(rpy90.apply({0., 0., 1.}),
                              drake::Vector3<double>(1., 0., 0.), kVeryExact));
}
...
TEST_P(MultilaneLanesParamTest, FlatLineLane) {
  ...
  std::unique_ptr<RoadCurve> road_curve_1 = std::make_unique<LineRoadCurve>(
      drake::Vector2<double>(100., -75.), drake::Vector2<double>(100., 50.), zp, zp,
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(
          drake::Vector3<double>(100. + ((100. / l1->length()) * 1.),
                          -75. + ((50. / l1->length()) * 1.), 0.) +
          r_offset_vector),
      kLinearTolerance));
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(
          drake::Vector3<double>(100. + ((-50. / l1->length()) * 3.),
  ...
  double distance{};
  const drake::Vector3<double> d_point_lane_origin = point_within_lane.xyz() -
                                              drake::Vector3<double>(100., -75., 0.) -
                                              r_offset_vector;
  ...
  const drake::Vector3<double> extreme_lane_point =
      drake::Vector3<double>(100., -75, 0.0) + r_offset_vector +
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(extreme_lane_point +
                                drake::Vector3<double>(0., 0., elevation)),
      kVeryExact));
...
class CorkScrew {
 public:
  ...
  drake::Vector3<T> motion_derivative_at_srh(const drake::Vector3<T>& srh,
...
drake::Vector3<T> CorkScrew<T>::position_at_srh(const drake::Vector3<T>& srh) const {
  ...
  return drake::Vector3<T>(axial_length_ * p,
                    effective_r_offset * cgamma - srh(2) * sgamma,
                    effective_r_offset * sgamma + srh(2) * cgamma);
}

template <typename T>
drake::Vector3<T> CorkScrew<T>::motion_derivative_at_srh(
    const drake::Vector3<T>& srh, const drake::Vector3<T>& iso_v) const {
  // TODO(hidmic): Assuming the LANE frame s-axis is always aligned
  //               with the GLOBAL frame x-axis is incorrect. However,
  //               this same bug is present in Multilane. Fix this
  //               computation when the implementation gets fixed.
  ...
  const drake::Vector3<T> position_derivative_at_p00(
      axial_length_, -alpha0 * sgamma, alpha0 * cgamma);
  const drake::Vector3<T> position_derivative_at_prh(
      axial_length_, -alpha * sgamma - beta * cgamma,
      alpha * cgamma - beta * sgamma);
  return iso_v.cwiseProduct(
      drake::Vector3<T>(position_derivative_at_p00.norm() /
...
drake::Vector3<T> CorkScrew<T>::orientation_at_srh(const drake::Vector3<T>& srh) const {
  ...
  const T beta =
      std::atan2(-s_hat.z(), drake::Vector2<T>(s_hat.x(), s_hat.y()).norm());
...
TEST_P(MultilaneLanesParamTest, CorkScrewLane) {
  ...
  std::unique_ptr<RoadCurve> road_curve =
      std::make_unique<LineRoadCurve>(
          drake::Vector2<double>(0., 0.), drake::Vector2<double>(kLength, 0.),
  ...
  const drake::Vector3<double> lane_velocity_as_vector(
...
TEST_P(MultilaneLanesParamTest, FlatArcLane) {
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(
          geo_center +
          drake::Vector3<double>(std::cos(theta0), std::sin(theta0), 0.0) *
              offset_radius),
      kLinearTolerance));
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(
          geo_center + drake::Vector3<double>((offset_radius - 1.) * std::cos(theta0),
  ...
  EXPECT_TRUE(api::test::IsGeoPositionClose(
      ...
      api::GeoPosition::FromXyz(
          geo_center +
          drake::Vector3<double>(offset_radius * std::cos(theta0 + d_theta),
...
TEST_P(MultilaneLanesParamTest, HillIntegration) {
  ...
  const CubicPolynomial kHillPolynomial(z0 / l_max, 0.,
                                        ...
                                        (-2. * (z1 - z0) / l_max));
  std::unique_ptr<RoadCurve> road_curve =
      std::make_unique<ArcRoadCurve>(drake::Vector2<double>(-100., -100.), radius,
    ...
    for (const double p : p_vector) {
      for (const double r : r_offset_vector) {
        const api::GeoPosition geo_point = api::GeoPosition::FromXyz(
            drake::Vector3<double>(100., -75., 0.) + (p * lane_length) * s_vector +
            (kR0 + lane_spacing + r) * r_vector);
        api::GeoPosition nearest_position;
        double distance{};
        EXPECT_TRUE(api::test::IsLanePositionClose(
...
TEST_F(MultilaneMultipleLanesTest, MultipleArcLanes) {
    ...
    for (const double p : p_vector) {
      for (const double r : r_offset_vector) {
        ...
        EXPECT_TRUE(api::test::IsGeoPositionClose(
            ...
            api::GeoPosition::FromXyz(
                kGeoCenter +
                effective_radius * drake::Vector3<double>(std::cos(effective_angle),
  ...
  for (size_t i = 0; i < lanes.size(); i++) {
    ...
    for (const double p : p_vector) {
      ...
      const api::GeoPosition geo_point = api::GeoPosition::FromXyz(
          kGeoCenter +
          kRadius * drake::Vector3<double>(std::cos(effective_angle),
                                    std::sin(effective_angle), 0.));
      const double expected_r = lane->driveable_bounds(0.).min();
      api::GeoPosition nearest_position;
      double distance{};
      ...
      EXPECT_TRUE(api::test::IsGeoPositionClose(
          ...
          api::GeoPosition::FromXyz(
              kGeoCenter +
              (kRadius - kRMin) * drake::Vector3<double>(std::cos(effective_angle),
                                                  std::sin(effective_angle),
                                                  0.)),
          kVeryExact));
      EXPECT_NEAR(distance, kRMin, kVeryExact);
