...
TEST_F(MultilaneBuilderReferenceCurvePrimitivesTest, LineSegment) {
  ...
  const drake::Vector3<double> r_versor(-std::sin(kStartHeading),
                                 std::cos(kStartHeading), 0.);
  const drake::Vector3<double> start_reference_curve(kStart.xy().x(), kStart.xy().y(),
                                              kStart.z().z());
...
TEST_F(MultilaneBuilderLaneToLanePrimitivesTest, FlatLineSegment) {
  ...
  const drake::Vector3<double> start_reference_curve =
      drake::Vector3<double>(kStart.xy().x(), kStart.xy().y(), kStart.z().z()) +
...
TEST_F(MultilaneBuilderLaneToLanePrimitivesTest, ElevatedEndLineSegment) {
  ...
  const drake::Vector3<double> end_reference_curve =
      start_reference_curve +
      kLength * drake::Vector3<double>(std::cos(kStartHeading),
  ...
  for (int i = 0; i < kNumLanes; i++) {
    ...
    // Checks lane start geo position to verify that spacing is correctly
    // applied.
    const drake::Vector3<double> r_offset =
        (-kRefR0 + static_cast<double>(i) * kLaneWidth) * r_versor;
    EXPECT_TRUE(api::test::IsGeoPositionClose(
        lane->ToGeoPosition({0., 0., 0.}),
...
TEST_F(MultilaneBuilderLaneToLanePrimitivesTest, ElevatedEndArcSegment) {
  ...
  const drake::Vector3<double> r_versor_start(-std::sin(kStartHeading),
                                       std::cos(kStartHeading), 0.);
  const drake::Vector3<double> start_reference_curve =
      drake::Vector3<double>(kStart.xy().x(), kStart.xy().y(), kStart.z().z()) +
      (kRefR0 - kLaneWidth) * r_versor_start;
  const drake::Vector3<double> r_versor_end(-std::sin(kStartHeading + kDTheta),
                                     std::cos(kStartHeading + kDTheta), 0.);
  const drake::Vector3<double> end_reference_curve =
      start_reference_curve + kRadius * drake::Vector3<double>(std::sqrt(2.), 0., 0.) +
      drake::Vector3<double>(0., 0., 5.);
  ...
  for (int i = 0; i < kNumLanes; i++) {
    ...
    // Checks lane start geo position to verify that spacing is correctly
    // applied.
    const drake::Vector3<double> r_offset_start =
    ...
    EXPECT_TRUE(api::test::IsGeoPositionClose(
        ...
        kLinearTolerance));
    // Checks lane end geo position to verify elevation is correctly applied.
    const drake::Vector3<double> r_offset_end =
 ...
 public:
  ...
  void AssertProperConstruction(
    ...
    for (double r = lbounds.min(); r <= lbounds.max(); r += lane_width/10) {
      ...
      const drake::Quaternion<double> pi_rotation(
          drake::AngleAxis<double>(M_PI, drake::Vector3<double>::UnitZ()));
