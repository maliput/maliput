...
RoadCurve::RoadCurve(double linear_tolerance, double scale_length,
  ...
  const drake::systems::AntiderivativeFunction<double>::SpecifiedValues
  ...
  const drake::systems::ScalarInitialValueProblem<double>::SpecifiedValues
...
drake::Vector3<double> RoadCurve::W_prime_of_prh(double p, double r, double h,
                                          const Rot3& Rabg,
  ...
  return
      drake::Vector3<double>(G_prime.x(),
                      G_prime.y(),
                      l_max() * g_prime) +
      ...
      drake::Vector3<double>(((sa*cb*cg)*r + (ca*cb*cg)*h),
...
Rot3 RoadCurve::Orientation(double p, double r, double h) const {
  ...
  const double beta =
      std::atan2(-s_hat.z(), drake::Vector2<double>(s_hat.x(), s_hat.y()).norm());
  const double cb = std::cos(beta);
...
drake::Vector3<double> RoadCurve::s_hat_of_prh(double p, double r, double h,
                                        const Rot3& Rabg,
                                        double g_prime) const {
...
drake::Vector3<double> RoadCurve::r_hat_of_Rabg(const Rot3& Rabg) const {
