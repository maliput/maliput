...
MultilaneOnrampMerge::BuildOnramp() const {
  auto rb = BuilderFactory().Make(rc_.lane_width, rc_.elevation_bounds,
  ...
  const LaneLayout lane_layout(rc_.left_shoulder, rc_.right_shoulder,
  ...
  const auto& post4 = rb->Connect(
      "post4", lane_layout,
      StartReference().at(*post5, LaneEnd::kFinish,
                                 Direction::kForward),
      ArcOffset(kPostArcLength, kPostArcRadius / kPostArcLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  const auto& post3 = rb->Connect(
      "post3", lane_layout,
      StartReference().at(*post4, LaneEnd::kFinish,
                                 Direction::kForward),
      ArcOffset(kPostArcLength, -kPostArcRadius / kPostArcLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  const auto& post2 = rb->Connect(
      "post2", lane_layout,
      StartReference().at(*post3, LaneEnd::kFinish,
                                 Direction::kForward),
      ArcOffset(kPostArcLength, kPostArcRadius / kPostArcLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  const auto& post1 = rb->Connect(
      "post1", lane_layout,
      StartReference().at(*post2, LaneEnd::kFinish,
                                 Direction::kForward),
      ArcOffset(kPostArcLength, -kPostArcRadius / kPostArcLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  const auto& post0 = rb->Connect(
      "post0", lane_layout,
      StartReference().at(*post1, LaneEnd::kFinish,
                                 Direction::kForward),
      ArcOffset(kPostArcLength, kPostArcRadius / kPostArcLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  ...
  const LaneLayout pre_lane_layout(rc_.left_shoulder, rc_.right_shoulder,
                                          pre_num_lanes, kRefLane, kRefR0);
  const auto& pre0 = rb->Connect(
      "pre0", pre_lane_layout,
      StartReference().at(*post0, LaneEnd::kFinish,
                                 Direction::kForward),
      LineOffset(kPostLinearLength),
      EndReference().z_at(kFlatZ, Direction::kForward));
  ...
  const auto& onramp0 = rb->Connect(
      ...
      StartReference().at(*onramp1, LaneEnd::kFinish,
                                 Direction::kForward),
      LineOffset(kOnrampLinearLength),
      EndReference().z_at(kFlatZ, Direction::kForward));

  // Manually specify the default branches for all junctions in the road.
  auto set_default_branches = [&](
      const Connection* in, int in_first_lane,
      const Connection* out, int out_first_lane, int num_lanes) {
