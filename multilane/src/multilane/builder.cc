...
const Connection* Builder::Connect(const std::string& id,
  ...
  const drake::Vector3<double> start_lane_position(start_spec.endpoint().xy().x(),
  ...
  const drake::Vector3<double> start_reference_position =
      start_lane_position +
      start_rotation.matrix() * drake::Vector3<double>(0., -start_lane_offset, 0.);
...
drake::Vector3<double> DirectionOutFromLane(const api::Lane* const lane,
...
bool AreLanesDirectionsWithinTolerance(
  ...
  for (int i = 0; i < set->size(); ++i) {
    ...
    const drake::Vector3<double> other_direction =
...
void Builder::AttachBranchPoint(
  ...
  const drake::Vector3<double> old_direction =
