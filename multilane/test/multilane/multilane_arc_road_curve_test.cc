...
TEST_F(MultilaneArcRoadCurveTest, ArcGeometryTest) {
  ...
  EXPECT_TRUE(CompareMatrices(
      dut.xy_of_p(0.0), kCenter + drake::Vector2<double>(kRadius * std::cos(kTheta0),
  ...
  EXPECT_TRUE(CompareMatrices(
      ...
      kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      dut.xy_of_p(1.0), kCenter + drake::Vector2<double>(kRadius * std::cos(kTheta1),
  ...
  EXPECT_TRUE(
      CompareMatrices(dut.xy_dot_of_p(0.0),
                      drake::Vector2<double>(-kRadius * std::sin(kTheta0) * kDTheta,
  ...
  EXPECT_TRUE(
      CompareMatrices(dut.xy_dot_of_p(1.0),
                      drake::Vector2<double>(-kRadius * std::sin(kTheta1) * kDTheta,
...
TEST_F(MultilaneArcRoadCurveTest, ToCurveFrameTest) {
  ...
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(
          drake::Vector3<double>(kCenter(0) + kRadius * std::cos(kTheta0),
  ...
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(
          drake::Vector3<double>(kCenter(0) + kRadius * std::cos(kTheta1),
  ...
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(
          drake::Vector3<double>(
              ...
              3.0),
          kRMin, kRMax, height_bounds),
      drake::Vector3<double>(0.75, 2.0, 3.0),
      kVeryExact));
}
    ...
    for (double r : r_vector) {
      for (double h : h_vector) {
        ...
        const drake::Vector3<double> geo_position =
            kGeoCenter +
            kRadius * drake::Vector3<double>(std::cos(angle), std::sin(angle), 0.) +
  ...
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        ...
        const drake::Vector3<double> w_prime = elevated_dut.W_prime_of_prh(
            p, r, h, rotation, g_prime);
        const drake::Vector3<double> numeric_w_prime =
            numeric_w_prime_of_prh(elevated_dut, p, r, h);
        EXPECT_TRUE(CompareMatrices(w_prime, numeric_w_prime, kQuiteExact));
        const drake::Vector3<double> s_hat = elevated_dut.s_hat_of_prh(
            p, r, h, rotation, g_prime);
        EXPECT_TRUE(CompareMatrices(w_prime.normalized(), s_hat, kVeryExact));
      }
    }
  ...
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        ...
        const drake::Vector3<double> w_prime = superelevated_dut.W_prime_of_prh(
            p, r, h, rotation, g_prime);
        const drake::Vector3<double> numeric_w_prime =
            numeric_w_prime_of_prh(superelevated_dut, p, r, h);
        EXPECT_TRUE(CompareMatrices(w_prime, numeric_w_prime, kQuiteExact));
        const drake::Vector3<double> s_hat = superelevated_dut.s_hat_of_prh(
            p, r, h, rotation, g_prime);
        EXPECT_TRUE(CompareMatrices(w_prime.normalized(), s_hat, kVeryExact));
      }
    }
...
TEST_F(MultilaneArcRoadCurveTest, ReferenceCurveRotation) {
  ...
  // Checks for a flat curve.
  {
    ...
    EXPECT_TRUE(CompareMatrices(
        r_hat, drake::Vector3<double>(-0.707106781186548, -0.707106781186548, 0.),
        kVeryExact));
    ...
    EXPECT_TRUE(
        CompareMatrices(r_hat, drake::Vector3<double>(0., -1., 0.), kVeryExact));
    ...
    r_hat = flat_dut.r_hat_of_Rabg(rotation);
    EXPECT_TRUE(CompareMatrices(
        r_hat, drake::Vector3<double>(0.382683432365090, -0.923879532511287, 0.),
        kVeryExact));
  ...
  // Checks for a curve with constant non zero superelevation.
  {
    ...
    EXPECT_TRUE(CompareMatrices(
        r_hat, drake::Vector3<double>(-0.353553390593274, -0.353553390593274,
                               0.866025403784439),
        kVeryExact));
    ...
    EXPECT_TRUE(CompareMatrices(
        r_hat, drake::Vector3<double>(0.0, -0.5, 0.866025403784439), kVeryExact));
    ...
    r_hat = superelevated_dut.r_hat_of_Rabg(rotation);
    EXPECT_TRUE(CompareMatrices(
        r_hat, drake::Vector3<double>(0.191341716182545, -0.461939766255643,
                               0.866025403784439),
        kVeryExact));
