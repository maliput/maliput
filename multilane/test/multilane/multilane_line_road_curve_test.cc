...
TEST_F(MultilaneLineRoadCurveTest, ToCurveFrameTest) {
  ...
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(drake::Vector3<double>(10.0, 10.0, 0.0), kRMin, kRMax,
                       elevation_bounds),
      drake::Vector3<double>(0.0, 0.0, 0.0), kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(drake::Vector3<double>(20.0, 20.0, 0.0), kRMin, kRMax,
                       elevation_bounds),
      drake::Vector3<double>(1., 0.0, 0.0), kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(drake::Vector3<double>(15.0, 15.0, 0.0), kRMin, kRMax,
                       elevation_bounds),
      drake::Vector3<double>(0.5, 0.0, 0.0), kVeryExact));
  // Check with lateral and vertical deviation.
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(drake::Vector3<double>(11.0, 12.0, 5.0), kRMin, kRMax,
                       elevation_bounds),
      drake::Vector3<double>(0.15, 0.707106781186547, 5.0), kVeryExact));
  EXPECT_TRUE(CompareMatrices(
      dut.ToCurveFrame(drake::Vector3<double>(11.0, 10.0, 7.0), kRMin, kRMax,
                       elevation_bounds),
...
TEST_F(MultilaneLineRoadCurveTest, WorldFunction) {
  ...
  const LineRoadCurve flat_dut(kOrigin, kDirection, zp, zp,
                               kLinearTolerance, kScaleLength,
                               kComputationPolicy);
  const drake::Vector3<double> p_versor =
      drake::Vector3<double>(kDirection.x(), kDirection.y(), 0.).normalized();
  const Rot3 flat_rotation(0., 0., kHeading);
  const drake::Vector3<double> kGeoOrigin(kOrigin.x(), kOrigin.y(), 0.);
  for (double p : p_vector) {
    for (double r : r_vector) {
      for (double h : h_vector) {
        const drake::Vector3<double> geo_position = kGeoOrigin +
...
TEST_F(MultilaneLineRoadCurveTest, ReferenceCurveRotation) {
  ...
  const LineRoadCurve superelevated_dut(kOrigin, kDirection, zp,
                                        ...
                                        kLinearTolerance, kScaleLength,
                                        kComputationPolicy);
  const drake::Vector3<double> kSuperelevatedRDirection{
    -10., 10., 10. * std::sqrt(2.)};
  for (double p : p_vector) {
    const Rot3 rotation = superelevated_dut.Rabg_of_p(p);
