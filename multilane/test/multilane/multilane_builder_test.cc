...
GTEST_TEST(StartReferenceSpecTest, Endpoint) {
  ...
  const StartReference::Spec forward_dut = StartReference().at(point, Direction::kForward);
  EXPECT_TRUE(
      test::IsEndpointClose(forward_dut.endpoint(), point, kEndpointLinearTolerance, kEndpointAngularTolerance));

  const StartReference::Spec reversed_dut = StartReference().at(point, Direction::kReverse);
  EXPECT_TRUE(test::IsEndpointClose(reversed_dut.endpoint(), point.reverse(), kEndpointLinearTolerance,
...
GTEST_TEST(StartReferenceSpecTest, Connection) {
  ...
  const Connection conn("conn", kStartEndpoint, kFlatZ, 2, 0., 1., 1.5, 1.5, kLineOffset, kLinearTolerance,
                        kScaleLength, kComputationPolicy);
  constexpr double kEndpointLinearTolerance{1e-15};
  constexpr double kEndpointAngularTolerance{1e-15};

  const StartReference::Spec forward_start_dut = StartReference().at(conn, Which::kStart, Direction::kForward);
  const Endpoint expected_forward_start_endpoint{kStartXy, kFlatZWithoutThetaDot};
  EXPECT_TRUE(test::IsEndpointClose(forward_start_dut.endpoint(), expected_forward_start_endpoint,
                                    kEndpointLinearTolerance, kEndpointAngularTolerance));

  const StartReference::Spec reversed_start_dut = StartReference().at(conn, Which::kStart, Direction::kReverse);
  const Endpoint expected_reversed_start_endpoint{kStartXy.reverse(), kFlatZWithoutThetaDot.reverse()};
  EXPECT_TRUE(test::IsEndpointClose(reversed_start_dut.endpoint(), expected_reversed_start_endpoint,
                                    kEndpointLinearTolerance, kEndpointAngularTolerance));

  const StartReference::Spec forward_end_dut = StartReference().at(conn, Which::kFinish, Direction::kForward);
  const Endpoint expected_forward_end_endpoint{kEndXy, kFlatZWithoutThetaDot};
  EXPECT_TRUE(test::IsEndpointClose(forward_end_dut.endpoint(), expected_forward_end_endpoint, kEndpointLinearTolerance,
                                    kEndpointAngularTolerance));

  const StartReference::Spec reversed_end_dut = StartReference().at(conn, Which::kFinish, Direction::kReverse);
  const Endpoint expected_reversed_end_endpoint{kEndXy.reverse(), kFlatZWithoutThetaDot.reverse()};
  EXPECT_TRUE(test::IsEndpointClose(reversed_end_dut.endpoint(), expected_reversed_end_endpoint,
                                    kEndpointLinearTolerance, kEndpointAngularTolerance));
}
...
GTEST_TEST(EndReferenceSpecTest, Endpoint) {
  ...
  const EndReference::Spec z_forward_dut = EndReference().z_at(z_point, Direction::kForward);
  EXPECT_TRUE(
      test::IsEndpointZClose(z_forward_dut.endpoint_z(), z_point, kEndpointLinearTolerance, kEndpointAngularTolerance));

  const EndReference::Spec z_reversed_dut = EndReference().z_at(z_point, Direction::kReverse);
  EXPECT_TRUE(test::IsEndpointZClose(z_reversed_dut.endpoint_z(), z_point.reverse(), kEndpointLinearTolerance,
                                     kEndpointAngularTolerance));
}
...
GTEST_TEST(EndReferenceSpecTest, Connection) {
  ...
  const Endpoint kStartEndpoint{kStartXy, kFlatZ};
  const EndpointZ kFlatZWithoutThetaDot{0., 0., 0., {}};
  const Connection conn("conn", kStartEndpoint, kFlatZ, 2, 0., 1., 1.5, 1.5, LineOffset(10.), kLinearTolerance,
                        kScaleLength, kComputationPolicy);
  constexpr double kEndpointLinearTolerance{1e-15};
  constexpr double kEndpointAngularTolerance{1e-15};

  const EndReference::Spec forward_start_dut = EndReference().z_at(conn, Which::kStart, Direction::kForward);
  const EndpointZ expected_forward_start_endpoint{kFlatZWithoutThetaDot};
  EXPECT_TRUE(test::IsEndpointZClose(forward_start_dut.endpoint_z(), kFlatZWithoutThetaDot, kEndpointLinearTolerance,
                                     kEndpointAngularTolerance));

  const EndReference::Spec reversed_start_dut = EndReference().z_at(conn, Which::kStart, Direction::kReverse);
  EXPECT_TRUE(test::IsEndpointZClose(reversed_start_dut.endpoint_z(), kFlatZWithoutThetaDot.reverse(),
                                     kEndpointLinearTolerance, kEndpointAngularTolerance));

  const EndReference::Spec forward_end_dut = EndReference().z_at(conn, Which::kFinish, Direction::kForward);
  EXPECT_TRUE(test::IsEndpointZClose(forward_end_dut.endpoint_z(), kFlatZWithoutThetaDot, kEndpointLinearTolerance,
                                     kEndpointAngularTolerance));

  const EndReference::Spec reversed_end_dut = EndReference().z_at(conn, Which::kFinish, Direction::kReverse);
  EXPECT_TRUE(test::IsEndpointZClose(reversed_end_dut.endpoint_z(), kFlatZWithoutThetaDot.reverse(),
                                     kEndpointLinearTolerance, kEndpointAngularTolerance));
}
...
GTEST_TEST(LaneLayeoutTest, ConstructorAndAccessors) {
  ...
  const LaneLayout dut(kLeftShoulder, kRightShoulder, kNumLanes, kRefLane, kRefR0);
...
GTEST_TEST(MultilaneBuilderTest, ProperConnections) {
  ...
  constexpr ComputationPolicy kComputationPolicy{ComputationPolicy::kPreferAccuracy};
  Builder builder(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance, kScaleLength, kComputationPolicy,
                  std::make_unique<GroupFactory>());
  ...
  const LaneLayout kLaneLayout(kLeftShoulder, kRightShoulder, kOneLane, kRefLane, kRefR0);
  ...
  const Connection* line_connection =
      builder.Connect("line", kLaneLayout, StartReference().at(kStartEndpoint, Direction::kForward), kLineOffset,
  ...
  EXPECT_TRUE(test::IsEndpointClose(line_connection->start(), kStartEndpoint, kEndpointLinearTolerance,
                                    kEndpointAngularTolerance));
  ...
  const Connection* arc_connection =
      builder.Connect("arc", kLaneLayout, StartReference().at(*line_connection, Which::kFinish, Direction::kForward),
  ...
  EXPECT_TRUE(test::IsEndpointClose(arc_connection->start(), line_connection->end(), kEndpointLinearTolerance,
                                    kEndpointAngularTolerance));
...
GTEST_TEST(MultilaneBuilderTest, Fig8) {
  ...
  const double kScaleLength = 1.0;
  const ComputationPolicy kComputationPolicy{ComputationPolicy::kPreferAccuracy};
  Builder b(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance, kScaleLength, kComputationPolicy,
            std::make_unique<GroupFactory>());
  ...
  auto c0 = b.Connect("0", kLaneLayout, StartReference().at(kStartEndpoint, Direction::kForward), kLineOffset,
                      EndReference().z_at(kMidFlatZ, Direction::kForward));

  auto c1 = b.Connect("1", kLaneLayout, StartReference().at(*c0, Which::kFinish, Direction::kForward),
                      kCounterClockwiseArc, EndReference().z_at(kMidTiltLeftZ, Direction::kForward));

  auto c2 = b.Connect("2", kLaneLayout, StartReference().at(*c1, Which::kFinish, Direction::kForward),
                      kCounterClockwiseArc, EndReference().z_at(kMidFlatZ, Direction::kForward));

  auto c3 = b.Connect("3", kLaneLayout, StartReference().at(*c2, Which::kFinish, Direction::kForward), kLineOffset,
                      EndReference().z_at(kHighFlatZ, Direction::kForward));

  auto c4 = b.Connect("4", kLaneLayout, StartReference().at(*c3, Which::kFinish, Direction::kForward), kLineOffset,
                      EndReference().z_at(kMidTiltRightZ, Direction::kForward));

  auto c5 = b.Connect("5", kLaneLayout, StartReference().at(*c4, Which::kFinish, Direction::kForward), kClockwiseArc,
                      EndReference().z_at(kMidTiltRightZ, Direction::kForward));

  auto c6 = b.Connect("6", kLaneLayout, StartReference().at(*c5, Which::kFinish, Direction::kForward), kClockwiseArc,
                      EndReference().z_at(kMidFlatZ, Direction::kForward));

  // Tweak ends to check if fuzzy-matching is working.
  Endpoint c6end = c6->end();
  c6end = Endpoint(
      EndpointXy(c6end.xy().x() + kLinearTolerance * 0.5, c6end.xy().y() - kLinearTolerance * 0.5,
                 c6end.xy().heading()),
      EndpointZ(c6end.z().z() + kLinearTolerance * 0.5, c6end.z().z_dot(), c6end.z().theta(), *c6end.z().theta_dot()));

  EndpointZ c0start_z = c0->start().z();
  c0start_z =
      EndpointZ(c0start_z.z() - kLinearTolerance * 0.5, c0start_z.z_dot(), c0start_z.theta(), *c0start_z.theta_dot());

  b.Connect("7", kLaneLayout, StartReference().at(c6end, Direction::kForward), kLineOffset,
            EndReference().z_at(c0start_z, Direction::kForward));

  std::unique_ptr<const api::RoadGeometry> rg = b.Build(api::RoadGeometryId{"figure-eight"});
...
GTEST_TEST(MultilaneBuilderTest, QuadRing) {
  ...
  auto left1 = b.Connect("left1", kLaneLayout, StartReference().at(kNorthbound, Direction::kReverse),
                         kLargeClockwiseLoop, EndReference().z_at(kFlatZ, Direction::kForward));
  // This heads +y, loops to -x, counterclockwise, back to origin.
  auto left0 = b.Connect("left0", kLaneLayout, StartReference().at(kNorthbound, Direction::kForward),
                         kSmallCounterClockwiseLoop, EndReference().z_at(kFlatZ, Direction::kForward));

  // This heads +y, loops to +x, clockwise, back to origin.
  auto right0 = b.Connect("right0", kLaneLayout, StartReference().at(kNorthbound, Direction::kForward),
                          kSmallClockwiseLoop, EndReference().z_at(kFlatZ, Direction::kForward));

  // This heads -y, loops to +x, counterclockwise, back to origin.
  auto right1 = b.Connect("right1", kLaneLayout, StartReference().at(kNorthbound, Direction::kReverse),
                          kLargeCounterClockwiseLoop, EndReference().z_at(kFlatZ, Direction::kForward));
  ...
  b.SetDefaultBranch(left1, kLaneIndex, api::LaneEnd::kStart, left1, kLaneIndex, api::LaneEnd::kFinish);
  b.SetDefaultBranch(left0, kLaneIndex, api::LaneEnd::kFinish, right0, kLaneIndex, api::LaneEnd::kStart);
  b.SetDefaultBranch(right0, kLaneIndex, api::LaneEnd::kFinish, right1, kLaneIndex, api::LaneEnd::kFinish);
  b.SetDefaultBranch(right1, kLaneIndex, api::LaneEnd::kStart, left1, kLaneIndex, api::LaneEnd::kFinish);

  b.SetDefaultBranch(left1, kLaneIndex, api::LaneEnd::kFinish, left1, kLaneIndex, api::LaneEnd::kStart);
  b.SetDefaultBranch(left0, kLaneIndex, api::LaneEnd::kStart, right1, kLaneIndex, api::LaneEnd::kStart);
  b.SetDefaultBranch(right0, kLaneIndex, api::LaneEnd::kStart, right1, kLaneIndex, api::LaneEnd::kStart);
  ...
  for (int si = 0; si < junction->num_segments(); ++si) {
    ...
    if (lane->id() == api::LaneId("l:left1_0")) {
      ...
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->lane->id(), api::LaneId("l:left1_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->end, api::LaneEnd::kFinish);
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->lane->id(), api::LaneId("l:left1_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->end, api::LaneEnd::kStart);
    } else if (lane->id() == api::LaneId("l:left0_0")) {
      EXPECT_EQ(lane->segment()->id(), api::SegmentId("s:left0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->lane->id(), api::LaneId("l:right1_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->end, api::LaneEnd::kStart);
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->lane->id(), api::LaneId("l:right0_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->end, api::LaneEnd::kStart);
    } else if (lane->id() == api::LaneId("l:right0_0")) {
      EXPECT_EQ(lane->segment()->id(), api::SegmentId("s:right0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->lane->id(), api::LaneId("l:right1_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kStart)->end, api::LaneEnd::kStart);
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->lane->id(), api::LaneId("l:right1_0"));
      EXPECT_EQ(lane->GetDefaultBranch(api::LaneEnd::kFinish)->end, api::LaneEnd::kFinish);
...
class MultilaneBuilderReferenceCurvePrimitivesTest : public ::testing::Test {
 protected:
  ...
  const int kRefLane{0};
  const LaneLayout kLaneLayout{kLeftShoulder, kRightShoulder, kNumLanes, kRefLane, kRefR0};
  const api::HBounds kElevationBounds{0., 5.};
  const double kLinearTolerance{0.01};
  const double kAngularTolerance{0.01 * M_PI};
  const double kScaleLength{1.0};
...
TEST_F(MultilaneBuilderReferenceCurvePrimitivesTest, LineSegment) {
  ...
  const LineOffset kLineOffset(50.);
  b.Connect("c0", kLaneLayout, StartReference().at(kStart, Direction::kForward), kLineOffset,
            EndReference().z_at(kLowFlatZ, Direction::kForward));

  std::unique_ptr<const api::RoadGeometry> rg = b.Build(api::RoadGeometryId{"multilane-line-segment"});
...
TEST_F(MultilaneBuilderReferenceCurvePrimitivesTest, ArcSegment) {
  ...
  b.Connect("c0", kLaneLayout, StartReference().at(kStart, Direction::kForward), ArcOffset(kRadius, kDTheta),
            EndReference().z_at(kLowFlatZ, Direction::kForward));

  std::unique_ptr<const api::RoadGeometry> rg = b.Build(api::RoadGeometryId{"multilane-arc-segment"});
...
// expression.
class MultilaneBuilderPrimitiveContinuityConstraintTest : public ::testing::Test {
 protected:
  const double kLaneWidth{4.};
  const double kRefR0{0.};
  const double kLeftShoulder{2.};
...
TEST_F(MultilaneBuilderPrimitiveContinuityConstraintTest, MonolaneLineSegment) {
  Builder b(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance, kScaleLength, kComputationPolicy,
            std::make_unique<GroupFactory>());
  const LineOffset kLineOffset(50.);
  auto c0 = b.Connect("c0", kLaneLayout, StartReference().at(kStartEndpoint, Direction::kForward), kLineOffset,
                      EndReference().z_at(kEndZ, Direction::kForward));
  EXPECT_NE(c0, nullptr);
  EXPECT_TRUE(test::IsEndpointClose(c0->start(), {kStartEndpoint.xy(), {1., 2., M_PI / 6., 0.}}, kLinearTolerance,
                                    kAngularTolerance));
  EXPECT_TRUE(test::IsEndpointClose(
      c0->end(),
      {{50. * std::cos(kStartHeading), 50. * std::sin(kStartHeading), kStartHeading}, {4., 5., -M_PI / 6., 0.}},
...
TEST_F(MultilaneBuilderPrimitiveContinuityConstraintTest, MonolaneArcSegment) {
  ...
  auto counter_clockwise_conn =
      b.Connect("counter_clockwise", kLaneLayout, StartReference().at(kStartEndpoint, Direction::kForward),
                ArcOffset(kRadius, kDTheta), EndReference().z_at(kEndZ, Direction::kForward));
  EXPECT_NE(counter_clockwise_conn, nullptr);
  EXPECT_TRUE(test::IsEndpointClose(counter_clockwise_conn->start(),
                                    {kStartEndpoint.xy(), {1., 2., M_PI / 6., -0.0298142396999972}}, kLinearTolerance,
                                    kAngularTolerance));
  EXPECT_TRUE(test::IsEndpointClose(
      counter_clockwise_conn->end(),
      {{kRadius * std::sqrt(2.), 0., kStartHeading + kDTheta}, {4., 5., -M_PI / 6., -0.0326860225230307}},
      kLinearTolerance, kAngularTolerance));

  auto clockwise_conn = b.Connect("clockwise", kLaneLayout, StartReference().at(kStartEndpoint, Direction::kForward),
                                  ArcOffset(kRadius, -kDTheta), EndReference().z_at(kEndZ, Direction::kForward));
  EXPECT_NE(clockwise_conn, nullptr);
  EXPECT_TRUE(test::IsEndpointClose(clockwise_conn->start(),
                                    {kStartEndpoint.xy(), {1., 2., M_PI / 6., 0.0298142396999972}}, kLinearTolerance,
                                    kAngularTolerance));
  EXPECT_TRUE(test::IsEndpointClose(
      clockwise_conn->end(),
      {{0., -kRadius * std::sqrt(2.), kStartHeading - kDTheta}, {4., 5., -M_PI / 6., 0.0326860225230307}},
      kLinearTolerance, kAngularTolerance));
}
...
TEST_F(MultilaneBuilderLaneToLanePrimitivesTest, FlatLineSegment) {
  ...
  const LineOffset kLineOffset(50.);
  b.Connect("c0", kLaneLayout, StartLane(kStartLane).at(kStart, Direction::kForward), kLineOffset,
...
TEST_F(MultilaneBuilderLaneToLanePrimitivesTest, ArcSegment) {
  ...
  const double kRadius = 30.;
  const double kDTheta = 0.5 * M_PI;
  b.Connect("c0", kLaneLayout, StartLane(kStartLane).at(kStart, Direction::kForward), ArcOffset(kRadius, kDTheta),
            EndLane(kEndLane).z_at(kLowFlatZ, Direction::kForward));
...
class BuildProcedure {
 public:
  ...
  virtual void AssertProperConstruction(const api::RoadGeometry& road_geometry) const = 0;
...
class TurnBuildProcedure : public BuildProcedure {
 public:
  ...
  //                                  subtended angle.
  explicit TurnBuildProcedure(const std::string& name, const Endpoint& start_endpoint, double straight_conn_length,
                              double curved_conn_radius, double curved_conn_angular_delta)
      : BuildProcedure(name),
        start_endpoint_(start_endpoint),
  ...
  void AssertProperConstruction(const api::RoadGeometry& road_geometry) const override {
    ASSERT_EQ(road_geometry.num_junctions(), 2);
    const api::Junction* straight_junction =
        road_geometry.ById().GetJunction(api::JunctionId{"j:" + std::string{kStraightConnName}});
    const api::Junction* curved_junction =
        road_geometry.ById().GetJunction(api::JunctionId{"j:" + std::string{kCurvedConnName}});
    ...
    const api::LaneEndSet* straight_lane_branches = straight_lane->GetOngoingBranches(api::LaneEnd::kStart);
    ...
    const api::LaneEndSet* curved_lane_branches = curved_lane->GetOngoingBranches(api::LaneEnd::kStart);
 ...
 protected:
  ...
  LineOffset line_offset() const { return LineOffset(straight_conn_length_); }

  // Returns curved connection arc spec.
  ArcOffset arc_offset() const { return ArcOffset(curved_conn_radius_, curved_conn_angular_delta_); }
...
class TurnUsingRefToRConn : public TurnBuildProcedure {
 public:
  ...
  void ApplyTo(Builder* builder) const override {
    ...
    const LaneLayout lane_layout{kLeftShoulder, kRightShoulder, kLaneNum, ref_lane, -(total_width - lane_width) / 2.};

    const Connection* straight_conn =
        builder->Connect(kStraightConnName, lane_layout, StartReference().at(start_endpoint(), Direction::kForward),
    ...
    const Connection* curved_conn = builder->Connect(
        kCurvedConnName, lane_layout, StartReference().at(*straight_conn, api::LaneEnd::kStart, Direction::kReverse),
...
class TurnUsingRefToRRef : public TurnBuildProcedure {
 public:
  ...
  void ApplyTo(Builder* builder) const override {
    ...
    const Connection* curved_conn =
        builder->Connect(kCurvedConnName, lane_layout, StartReference().at(start_endpoint(), Direction::kReverse),
                         arc_offset(), EndReference().z_at(kFlatZ, Direction::kForward));
    ASSERT_TRUE(curved_conn != nullptr);
  }
};
...
class TurnUsingStraightRefToCurvedRLane : public TurnBuildProcedure {
 public:
  ...
  void ApplyTo(Builder* builder) const override {
    ...
    const Connection* curved_conn =
        builder->Connect(kCurvedConnName, lane_layout,
                         StartLane(ref_lane).at(*straight_conn, other_lane, api::LaneEnd::kStart, Direction::kReverse),
                         arc_offset(), EndLane(ref_lane).z_at(kFlatZ, Direction::kForward));
    ASSERT_TRUE(curved_conn != nullptr);
  }
};
...
class TurnUsingCurvedRefToStraightRLane : public TurnBuildProcedure {
 public:
  ...
  void ApplyTo(Builder* builder) const override {
    ...
    const Connection* curved_conn =
        builder->Connect(kCurvedConnName, lane_layout, StartReference().at(start_endpoint(), Direction::kForward),
                         arc_offset(), EndReference().z_at(kFlatZ, Direction::kForward));
    ASSERT_TRUE(curved_conn != nullptr);
    ...
    const Connection* straight_conn =
        builder->Connect(kStraightConnName, lane_layout,
                         StartLane(ref_lane).at(*curved_conn, other_lane, api::LaneEnd::kStart, Direction::kReverse),
                         line_offset(), EndLane(ref_lane).z_at(kFlatZ, Direction::kForward));
    ASSERT_TRUE(straight_conn != nullptr);
  }
};
...
using TestWithBuildProcedure = ::testing::TestWithParam<std::shared_ptr<BuildProcedure>>;
...
class MultilaneDiscontinuousBuildProcedureTest : public MultilaneBuildProcedureTest {};
...
TEST_P(MultilaneDiscontinuousBuildProcedureTest, ThrowingUponBuild) {
  ...
  EXPECT_THROW(
      {
        const std::unique_ptr<const api::RoadGeometry> road_geometry = builder.Build(api::RoadGeometryId{"bad_road"});
      },
...
// issues.
std::vector<std::shared_ptr<BuildProcedure>> ListBadReverseBuildProcedures() {
  ...
  procedures.push_back(std::make_unique<TurnUsingRefToRConn>("BankedTurnUsingRefToRConnWithBadThetaDot",
                                                             kBadStartEndpoint, kStraightConnLength, kCurvedConnRadius,
                                                             kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingRefToRRef>("BankedTurnUsingRefToRRefWithBadThetaDot",
                                                            kBadStartEndpoint, kStraightConnLength, kCurvedConnRadius,
                                                            kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingStraightRefToCurvedRLane>(
      "BankedTurnUsingStraightRefToCurvedRLaneWithBadThetaDot", kBadStartEndpoint, kStraightConnLength,
      kCurvedConnRadius, kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingCurvedRefToStraightRLane>(
      "BankedTurnUsingCurvedRefToStraightRLaneWithBadThetaDot", kBadStartEndpoint, kStraightConnLength,
      kCurvedConnRadius, kCurvedConnAngularDelta));
  return procedures;
}

INSTANTIATE_TEST_CASE_P(BadReverseConnectionTest, MultilaneDiscontinuousBuildProcedureTest,
...
class MultilaneContinuousBuildProcedureTest : public MultilaneBuildProcedureTest {};
...
TEST_P(MultilaneContinuousBuildProcedureTest, Continuity) {
  Builder builder(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance, kScaleLength, kComputationPolicy,
                  std::make_unique<GroupFactory>());
  ApplyProcedureTo(&builder);
  const std::unique_ptr<const api::RoadGeometry> road_geometry = builder.Build(api::RoadGeometryId{"good_road"});
  AssertProcedureResult(*road_geometry);
}

// Returns a collection of BuildProcedure instances that make use
// of reversed semantics in a way that produces G1 continuous roads.
std::vector<std::shared_ptr<BuildProcedure>> ListGoodReverseBuildProcedures() {
  constexpr double kStraightConnLength{50.};
  constexpr double kCurvedConnRadius{50.};
  constexpr double kCurvedConnAngularDelta{M_PI / 2.};
  const Endpoint kStartEndpoint{{0., 0., M_PI / 2.}, {0., 0., 0.3, {}}};
  std::vector<std::shared_ptr<BuildProcedure>> procedures;
  procedures.push_back(std::make_unique<TurnUsingRefToRConn>("BankedTurnUsingRefToRConnWithoutThetaDot", kStartEndpoint,
                                                             kStraightConnLength, kCurvedConnRadius,
                                                             kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingRefToRRef>("BankedTurnUsingRefToRRefWithoutThetaDot", kStartEndpoint,
                                                            kStraightConnLength, kCurvedConnRadius,
                                                            kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingStraightRefToCurvedRLane>(
      "BankedTurnUsingStraightRefToCurvedRLaneWithoutThetaDot", kStartEndpoint, kStraightConnLength, kCurvedConnRadius,
      kCurvedConnAngularDelta));
  procedures.push_back(std::make_unique<TurnUsingCurvedRefToStraightRLane>(
      "BankedTurnUsingCurvedRefToStraightRLaneWithoutThetaDot", kStartEndpoint, kStraightConnLength, kCurvedConnRadius,
      kCurvedConnAngularDelta));
  return procedures;
}

INSTANTIATE_TEST_CASE_P(GoodReverseConnectionTest, MultilaneContinuousBuildProcedureTest,
...
class MultilaneBuilderMultilaneCrossTest : public ::testing::Test {
 protected:
  ...
  const Endpoint endpoint_a_without_theta_dot{{0., 0., 0.}, kLowFlatZWithoutThetaDot};
  ...
  const Endpoint endpoint_d_without_theta_dot{{60., 50., -M_PI / 2.}, kLowFlatZWithoutThetaDot};
  ...
  const std::map<std::string, std::vector<std::string>> segment_truth_map{
      {"s:c1", {"l:c1_0", "l:c1_1", "l:c1_2"}}, {"s:c2", {"l:c2_0", "l:c2_1"}},          {"s:c3", {"l:c3_0", "l:c3_1"}},
      {"s:c4", {"l:c4_0", "l:c4_1", "l:c4_2"}}, {"s:c5", {"l:c5_0", "l:c5_1"}},          {"s:c6", {"l:c6_0", "l:c6_1"}},
      {"s:c7", {"l:c7_0", "l:c7_1"}},           {"s:c8", {"l:c8_0", "l:c8_1", "l:c8_2"}}};
  ...
  const std::map<std::string, BranchPointLaneIds> lane_truth_map{
      ...
      {"l:c2_0", {{"l:c2_0", "l:c4_1"}, {"l:c1_1"}, {"l:c2_0", "l:c6_0"}, {"l:c3_0"}}},
      {"l:c2_1", {{"l:c2_1", "l:c4_2"}, {"l:c1_2"}, {"l:c2_1", "l:c6_1"}, {"l:c3_1"}}},
      {"l:c3_0", {{"l:c2_0", "l:c6_0"}, {"l:c3_0"}, {"l:c3_0"}, {}}},
      {"l:c3_1", {{"l:c2_1", "l:c6_1"}, {"l:c3_1"}, {"l:c3_1"}, {}}},
      {"l:c4_0", {{"l:c4_0"}, {"l:c1_0"}, {"l:c4_0"}, {"l:c8_0"}}},
      {"l:c4_1", {{"l:c2_0", "l:c4_1"}, {"l:c1_1"}, {"l:c4_1", "l:c7_0"}, {"l:c8_1"}}},
      {"l:c4_2", {{"l:c2_1", "l:c4_2"}, {"l:c1_2"}, {"l:c4_2", "l:c7_1"}, {"l:c8_2"}}},
      {"l:c5_0", {{"l:c5_0"}, {}, {"l:c6_0", "l:c7_0"}, {"l:c5_0"}}},
      {"l:c5_1", {{"l:c5_1"}, {}, {"l:c6_1", "l:c7_1"}, {"l:c5_1"}}},
      {"l:c6_0", {{"l:c6_0", "l:c7_0"}, {"l:c5_0"}, {"l:c2_0", "l:c6_0"}, {"l:c3_0"}}},
      {"l:c6_1", {{"l:c6_1", "l:c7_1"}, {"l:c5_1"}, {"l:c2_1", "l:c6_1"}, {"l:c3_1"}}},
      {"l:c7_0", {{"l:c6_0", "l:c7_0"}, {"l:c5_0"}, {"l:c4_1", "l:c7_0"}, {"l:c8_1"}}},
      {"l:c7_1", {{"l:c6_1", "l:c7_1"}, {"l:c5_1"}, {"l:c4_2", "l:c7_1"}, {"l:c8_2"}}},
  ...
  void BuildAndCheckRoadGeometry(Builder* b) {
    std::unique_ptr<const api::RoadGeometry> rg = b->Build(api::RoadGeometryId{"multilane-cross"});
...
TEST_F(MultilaneBuilderMultilaneCrossTest, MultilaneRefCurveToRefCurve) {
  Builder b(kLaneWidth, kElevationBounds, kLinearTolerance, kAngularTolerance, kScaleLength, kComputationPolicy,
            std::make_unique<GroupFactory>());
  // Creates connections.
  b.Connect("c1", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 0.),
            StartReference().at(endpoint_a, Direction::kForward), LineOffset(50.),
            EndReference().z_at(kLowFlatZ, Direction::kReverse));

  auto c2 = b.Connect("c2", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 4.),
                      StartReference().at(endpoint_b, Direction::kForward), LineOffset((20.)),
                      EndReference().z_at(kLowFlatZ, Direction::kReverse));

  b.Connect("c3", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 4.),
            StartReference().at(endpoint_c, Direction::kForward), LineOffset(30.),
            EndReference().z_at(kLowFlatZ, Direction::kReverse));

  auto c4 = b.Connect("c4", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 0.),
                      StartReference().at(endpoint_b, Direction::kForward), ArcOffset(6., -M_PI / 2.),
                      EndReference().z_at(kLowFlatZ, Direction::kReverse));

  b.Connect("c5", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 0.),
            StartReference().at(endpoint_d, Direction::kForward), LineOffset(36.),
            EndReference().z_at(kLowFlatZ, Direction::kReverse));

  auto c6 = b.Connect("c6", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 0.),
                      StartReference().at(endpoint_f, Direction::kForward), ArcOffset(10., M_PI / 2.),
                      EndReference().z_at(kLowFlatZ, Direction::kReverse));

  auto c7 = b.Connect("c7", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 10.),
                      StartReference().at(endpoint_e, Direction::kForward), LineOffset(20.),
                      EndReference().z_at(kLowFlatZ, Direction::kReverse));

  b.Connect("c8", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 6.),
...
TEST_F(MultilaneBuilderMultilaneCrossTest, MultilaneLaneCurveToLaneCurve) {
  ...
  // Creates connections.
  auto c1 = b.Connect("c1", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 0.),
                      StartLane(kFirstLane).at(endpoint_a_without_theta_dot, Direction::kForward), LineOffset(50.),
                      EndLane(kFirstLane).z_at(kLowFlatZWithoutThetaDot, Direction::kReverse));

  auto c2 = b.Connect("c2", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 4.),
                      StartLane(kFirstLane).at(*c1, kSecondLane, Which::kFinish, Direction::kForward), LineOffset(20.),
                      EndLane(kFirstLane).z_at(kLowFlatZWithoutThetaDot, Direction::kReverse));

  auto c3 = b.Connect("c3", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 4.),
                      StartLane(kSecondLane).at(*c2, kSecondLane, Which::kFinish, Direction::kForward), LineOffset(30.),
                      EndLane(kFirstLane).z_at(kLowFlatZWithoutThetaDot, Direction::kReverse));

  auto c4 =
      b.Connect("c4", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 0.),
                StartLane(kSecondLane).at(*c2, kFirstLane, Which::kStart, Direction::kForward),
                ArcOffset(6., -M_PI / 2.), EndLane(kThirdLane).z_at(kLowFlatZWithoutThetaDot, Direction::kReverse));

  auto c5 = b.Connect("c5", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 0.),
                      StartLane(kFirstLane).at(endpoint_d_without_theta_dot, Direction::kForward), LineOffset(36.),
                      EndLane(kFirstLane).z_at(kLowFlatZWithoutThetaDot, Direction::kReverse));

  auto c6 = b.Connect("c6", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 0.),
                      StartLane(kFirstLane).at(*c5, kFirstLane, Which::kFinish, Direction::kForward),
                      ArcOffset(10., M_PI / 2.),
                      EndLane(kSecondLane).z_at(*c3, kSecondLane, Which::kStart, Direction::kForward));

  auto c7 = b.Connect("c7", LaneLayout(kLeftShoulder, kRightShoulder, kTwoLanes, kRefLane, 10.),
                      StartLane(kFirstLane).at(*c5, kFirstLane, Which::kFinish, Direction::kForward), LineOffset(20.),
                      EndLane(kFirstLane).z_at(*c4, kSecondLane, Which::kFinish, Direction::kForward));

  b.Connect("c8", LaneLayout(kLeftShoulder, kRightShoulder, kThreeLanes, kRefLane, 6.),
            StartLane(kFirstLane).at(*c4, kFirstLane, Which::kFinish, Direction::kForward), LineOffset(44.),
