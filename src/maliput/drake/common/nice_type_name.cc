...
string NiceTypeName::Canonicalize(const string& demangled) {
  ...
  static const never_destroyed<std::vector<SPair>> subs{SPairList{
      ...
      // ... square matrices ...
      SPair(std::regex("\\bEigen::Matrix<([^,<>]*),(-?\\d+),\\2,0,\\2,\\2>"), "drake::Matrix$2<$1>"),
      // ... vectors ...
      SPair(std::regex("\\bEigen::Matrix<([^,<>]*),(-?\\d+),1,0,\\2,1>"), "drake::Vector$2<$1>"),
      ...
      SPair(std::regex("drake::(Vector|Matrix)(X|\\d+)"
            ...
            "Eigen::$1$2$4$5$6"),
      // ... AutoDiff.
      SPair(std::regex("Eigen::AutoDiffScalar<Eigen::VectorXd>"), "drake::AutoDiffXd"),
  }};
...
string NiceTypeName::RemoveNamespaces(const string& canonical) {
  ...
  const std::string no_namespace = std::regex_replace(canonical, regex.access(), "");

  return no_namespace.empty() ? canonical : no_namespace;
}

std::string NiceTypeName::GetWithPossibleOverride(const void* ptr, const std::type_info& info) {
  internal::NiceTypeNamePtrOverride ptr_override = internal::GetNiceTypeNamePtrOverride();
  if (ptr_override) {
