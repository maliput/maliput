...
void CacheEntryValue::ThrowIfBadCacheEntryValue(const internal::ContextMessageInterface* owning_subcontext) const {
  if (owning_subcontext_ == nullptr) {
    // Can't use FormatName() here because that depends on us having an owning
    // context to talk to.
    throw std::logic_error("CacheEntryValue(" + description() + ")::" + __func__ +
                           "(): entry has no owning subcontext.");
  }
  ...
  if ((flags_ & ~(kValueIsOutOfDate | kCacheEntryIsDisabled)) != 0) {
    throw std::logic_error(FormatName(__func__) + "flags value is out of range.");
  }
  if (serial_number() < 0) {
    throw std::logic_error(FormatName(__func__) + "serial number is negative.");
  }
  if (!(cache_index_.is_valid() && ticket_.is_valid())) {
    throw std::logic_error(FormatName(__func__) + "cache index or dependency ticket invalid.");
  }
}

void CacheEntryValue::ThrowIfBadOtherValue(const char* api,
                                           const std::unique_ptr<AbstractValue>* other_value_ptr) const {
  if (other_value_ptr == nullptr) throw std::logic_error(FormatName(api) + "null other_value pointer.");

  auto& other_value = *other_value_ptr;
  if (other_value == nullptr) throw std::logic_error(FormatName(api) + "other_value is empty.");

  MALIPUT_DRAKE_DEMAND(value_ != nullptr);  // Should have been checked already.

  if (value_->type_info() != other_value->type_info()) {
    throw std::logic_error(FormatName(api) + "other_value has wrong concrete type " + other_value->GetNiceTypeName() +
                           ". Expected " + value_->GetNiceTypeName() + ".");
  }
}

CacheEntryValue& Cache::CreateNewCacheEntryValue(CacheIndex index, DependencyTicket ticket,
  ...
  if (index >= cache_size()) store_.resize(index + 1);
  ...
  store_[index] = std::unique_ptr<CacheEntryValue>(
      new CacheEntryValue(index, ticket, description, owning_subcontext_, nullptr /* no value yet */));
  ...
  if (trackers->has_tracker(ticket)) {
    ...
    tracker = &trackers->CreateNewDependencyTracker(ticket, "cache " + description, &value);
  }
...
void Cache::RepairCachePointers(const internal::ContextMessageInterface* owning_subcontext) {
