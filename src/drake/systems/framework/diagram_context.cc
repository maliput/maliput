...
DiagramContext<T>::DiagramContext(int num_subcontexts)
    : contexts_(num_subcontexts), state_(std::make_unique<DiagramState<T>>(num_subcontexts)) {}
...
template <typename T>
void DiagramContext<T>::SubscribeExportedInputPortToDiagramPort(InputPortIndex input_port_index,
  ...
  MALIPUT_DRAKE_DEMAND(0 <= subsystem_iport_index && subsystem_iport_index < subcontext.num_input_ports());

  // Get this Diagram's input port that serves as the source.
  const DependencyTicket iport_ticket = this->input_port_ticket(input_port_index);
  DependencyTracker& iport_tracker = this->get_mutable_tracker(iport_ticket);
  const DependencyTicket subcontext_iport_ticket = subcontext.input_port_ticket(subsystem_iport_index);
  DependencyTracker& subcontext_iport_tracker = subcontext.get_mutable_tracker(subcontext_iport_ticket);
  subcontext_iport_tracker.SubscribeToPrerequisite(&iport_tracker);
}

template <typename T>
void DiagramContext<T>::SubscribeDiagramPortToExportedOutputPort(OutputPortIndex output_port_index,
  ...
  Context<T>& subcontext = GetMutableSubsystemContext(subsystem_index);
  MALIPUT_DRAKE_DEMAND(0 <= subsystem_oport_index && subsystem_oport_index < subcontext.num_output_ports());

  // Get the child subsystem's output port tracker that serves as the source.
  const DependencyTicket subcontext_oport_ticket = subcontext.output_port_ticket(subsystem_oport_index);
  DependencyTracker& subcontext_oport_tracker = subcontext.get_mutable_tracker(subcontext_oport_ticket);

  // Get the diagram's output port tracker that is the destination.
  const DependencyTicket oport_ticket = this->output_port_ticket(output_port_index);
...
void DiagramContext<T>::SubscribeInputPortToOutputPort(const OutputPortIdentifier& output_port,
  ...
  const DependencyTicket oport_ticket = oport_context.output_port_ticket(oport_index);
  const DependencyTicket iport_ticket = iport_context.input_port_ticket(iport_index);
  DependencyTracker& oport_tracker = oport_context.get_mutable_tracker(oport_ticket);
  DependencyTracker& iport_tracker = iport_context.get_mutable_tracker(iport_ticket);
...
void DiagramContext<T>::SubscribeDiagramCompositeTrackersToChildrens() {
  ...
  std::vector<internal::BuiltInTicketNumbers> composites{
      ...
      internal::kVTicket,     internal::kZTicket,  internal::kXdTicket, internal::kXaTicket,
      internal::kPnTicket,    internal::kPaTicket,
      internal::kXcdotTicket,  // Cache entries.
      internal::kPeTicket,    internal::kKeTicket, internal::kPcTicket, internal::kPncTicket};
  ...
  for (auto ticket : composites) {
    diagram_trackers.push_back(&graph.get_mutable_tracker(DependencyTicket(ticket)));
  }
  ...
  for (auto& subcontext : contexts_) {
    ...
    for (size_t i = 0; i < composites.size(); ++i) {
      auto& sub_tracker = subgraph.get_mutable_tracker(DependencyTicket(composites[i]));
...
void DiagramContext<T>::MakeParameters() {
  ...
  for (auto& subcontext : contexts_) {
    // Using `access` here to avoid sending invalidations.
    Parameters<T>& subparams = Context<T>::access_mutable_parameters(&*subcontext);
  ...
  params->set_numeric_parameters(std::make_unique<DiscreteValues<T>>(numeric_params));
  params->set_abstract_parameters(std::make_unique<AbstractValues>(abstract_params));
...
std::string DiagramContext<T>::do_to_string() const {
  ...
  if (this->num_continuous_states()) os << this->num_continuous_states() << " total continuous states\n";
  if (this->num_discrete_state_groups()) {
    ...
    os << num_discrete_states << " total discrete states in " << this->num_discrete_state_groups() << " groups\n";
  }
  if (this->num_abstract_states()) os << this->num_abstract_states() << " total abstract states\n";
  ...
  if (this->num_numeric_parameter_groups()) {
    ...
    os << num_numeric_parameters << " total numeric parameters in " << this->num_numeric_parameter_groups()
       << " groups\n";
  }
  if (this->num_abstract_parameters()) os << this->num_abstract_parameters() << " total abstract parameters\n";
  ...
  for (systems::SubsystemIndex i{0}; i < num_subcontexts(); i++) {
    ...
    if (subcontext.get_continuous_state_vector().size() || subcontext.num_discrete_state_groups() ||
        subcontext.num_abstract_states() || subcontext.num_numeric_parameter_groups() ||
...
template <typename T>
void DiagramContext<T>::DoPropagateTimeChange(const T& time_sec, const std::optional<T>& true_time,
                                              int64_t change_event) {
  for (auto& subcontext : contexts_) {
    MALIPUT_DRAKE_ASSERT(subcontext != nullptr);
    Context<T>::PropagateTimeChange(&*subcontext, time_sec, true_time, change_event);
  }
}

template <typename T>
void DiagramContext<T>::DoPropagateAccuracyChange(const std::optional<double>& accuracy, int64_t change_event) {
  for (auto& subcontext : contexts_) {
    MALIPUT_DRAKE_ASSERT(subcontext != nullptr);
...
template <typename T>
void DiagramContext<T>::DoPropagateBulkChange(int64_t change_event,
                                              void (ContextBase::*note_bulk_change)(int64_t change_event)) {
  for (auto& subcontext : contexts_) {
    MALIPUT_DRAKE_ASSERT(subcontext != nullptr);
    ContextBase::PropagateBulkChange(&*subcontext, change_event, note_bulk_change);
  }
}

template <typename T>
void DiagramContext<T>::DoPropagateCachingChange(void (Cache::*caching_change)()) const {
  for (auto& subcontext : contexts_) {
    MALIPUT_DRAKE_ASSERT(subcontext != nullptr);
...
template <typename T>
void DiagramContext<T>::DoPropagateBuildTrackerPointerMap(const ContextBase& clone,
  ...
  for (SubsystemIndex i(0); i < num_subcontexts(); ++i) {
    ContextBase::BuildTrackerPointerMap(*contexts_[i], *clone_diagram.contexts_[i], &*tracker_map);
  }
}

template <typename T>
void DiagramContext<T>::DoPropagateFixContextPointers(const ContextBase& source,
  ...
  for (SubsystemIndex i(0); i < num_subcontexts(); ++i) {
    ContextBase::FixContextPointers(*source_diagram.contexts_[i], tracker_map, &*contexts_[i]);
  }
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::maliput::drake::systems::DiagramContext)
