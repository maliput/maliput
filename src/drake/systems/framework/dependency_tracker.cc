...
void DependencyTracker::NoteValueChange(int64_t change_event) const {
  DRAKE_LOGGER_DEBUG("Tracker '{}' value change event {} ...", GetPathDescription(), change_event);
  ...
  if (last_change_event_ == change_event) {
    ...
    DRAKE_LOGGER_DEBUG("... ignoring repeated value change notification same change event.");
    return;
  }
...
void DependencyTracker::NotePrerequisiteChange(int64_t change_event, const DependencyTracker& prerequisite,
                                               int depth) const {
  unused(Indent);  // Avoid warning in non-Debug builds.
  DRAKE_LOGGER_DEBUG("{}Tracker '{}': prerequisite '{}' changed (event {}) ...", Indent(depth), GetPathDescription(),
  ...
  if (last_change_event_ == change_event) {
    ++num_ignored_notifications_;
    DRAKE_LOGGER_DEBUG("{}... ignoring repeated prereq change notification same change event.", Indent(depth));
    return;
  }
  last_change_event_ = change_event;
...
void DependencyTracker::NotifySubscribers(int64_t change_event, int depth) const {
  DRAKE_LOGGER_DEBUG("{}... {} downstream subscribers.{}", Indent(depth), num_subscribers(),
  ...
  for (const DependencyTracker* subscriber : subscribers_) {
    ...
    DRAKE_LOGGER_DEBUG("{}->{}", Indent(depth), subscriber->GetPathDescription());
...
void DependencyTracker::SubscribeToPrerequisite(DependencyTracker* prerequisite) {
  MALIPUT_DRAKE_DEMAND(prerequisite != nullptr);
  DRAKE_LOGGER_DEBUG("Tracker '{}' subscribing to prerequisite '{}'", GetPathDescription(),
                     prerequisite->GetPathDescription());
...
void DependencyTracker::AddDownstreamSubscriber(const DependencyTracker& subscriber) {
  DRAKE_LOGGER_DEBUG("Tracker '{}' adding subscriber '{}'", GetPathDescription(), subscriber.GetPathDescription());
...
bool Contains(const T& value, const std::vector<T>& to_search) {
  return std::find(to_search.begin(), to_search.end(), value) != to_search.end();
}
...
void Remove(const T& value, std::vector<T>* to_search) {
  ...
  MALIPUT_DRAKE_DEMAND(found != to_search->end());
  to_search->erase(found);
}
}  // namespace

// Remove a subscription that we made earlier.
void DependencyTracker::UnsubscribeFromPrerequisite(DependencyTracker* prerequisite) {
  MALIPUT_DRAKE_DEMAND(prerequisite != nullptr);
  DRAKE_LOGGER_DEBUG("Tracker '{}' unsubscribing from prerequisite '{}'", GetPathDescription(),
                     prerequisite->GetPathDescription());
...
void DependencyTracker::RemoveDownstreamSubscriber(const DependencyTracker& subscriber) {
  DRAKE_LOGGER_DEBUG("Tracker '{}' removing subscriber '{}'", GetPathDescription(), subscriber.GetPathDescription());
...
std::string DependencyTracker::GetPathDescription() const { return GetSystemPathname() + ":" + description(); }

bool DependencyTracker::HasPrerequisite(const DependencyTracker& prerequisite) const {
  return Contains(&prerequisite, prerequisites_);
}

bool DependencyTracker::HasSubscriber(const DependencyTracker& subscriber) const {
  return Contains(&subscriber, subscribers_);
}

void DependencyTracker::ThrowIfBadDependencyTracker(const internal::ContextMessageInterface* owning_subcontext,
  ...
  if (owning_subcontext_ == nullptr) {
    ...
    // context to talk to.
    throw std::logic_error("DependencyTracker(" + description() + ")::" + __func__ +
  ...
  if (cache_value_ == nullptr) {
    throw std::logic_error(FormatName(__func__) + "no associated cache entry value (should at least be a dummy).");
  }
  if (cache_value && cache_value_ != cache_value) {
    throw std::logic_error(FormatName(__func__) + "wrong associated cache entry value.");
  }
  if (!ticket_.is_valid()) {
    throw std::logic_error(FormatName(__func__) + "dependency ticket invalid.");
  }
  if (last_change_event_ < -1) {
    throw std::logic_error(FormatName(__func__) + "last change event has an absurd value.");
  }
  if (num_value_change_notifications_received_ < 0 || num_prerequisite_notifications_received_ < 0 ||
      num_ignored_notifications_ < 0 || num_downstream_notifications_sent_ < 0) {
    throw std::logic_error(FormatName(__func__) + "a counter has a negative value.");
  }
}

void DependencyTracker::RepairTrackerPointers(const DependencyTracker& source,
  ...
  MALIPUT_DRAKE_DEMAND(cache != nullptr);
  owning_subcontext_ = owning_subcontext;

  // Set the cache entry pointer.
  MALIPUT_DRAKE_DEMAND(has_associated_cache_entry_ == source.has_associated_cache_entry_);
  if (has_associated_cache_entry_) {
    ...
    DRAKE_LOGGER_DEBUG("Cloned tracker '{}' repairing cache entry {} invalidation to {:#x}.", GetPathDescription(),
                       source.cache_value_->cache_index(), size_t(cache_value_));
  }

  // Set the subscriber pointers.
  MALIPUT_DRAKE_DEMAND(num_subscribers() == source.num_subscribers());
  for (int i = 0; i < num_subscribers(); ++i) {
    MALIPUT_DRAKE_ASSERT(subscribers_[i] == nullptr);
    auto map_entry = tracker_map.find(source.subscribers()[i]);
    MALIPUT_DRAKE_DEMAND(map_entry != tracker_map.end());
    subscribers_[i] = map_entry->second;
  }

  // Set the prerequisite pointers.
  MALIPUT_DRAKE_DEMAND(num_prerequisites() == source.num_prerequisites());
...
void DependencyGraph::AppendToTrackerPointerMap(const DependencyGraph& clone,
                                                DependencyTracker::PointerMap* tracker_map) const {
  MALIPUT_DRAKE_DEMAND(tracker_map != nullptr);
  MALIPUT_DRAKE_DEMAND(clone.trackers_size() == trackers_size());
  for (DependencyTicket ticket(0); ticket < trackers_size(); ++ticket) {
    if (!has_tracker(ticket)) continue;
    const bool added = tracker_map->emplace(&get_tracker(ticket), &clone.get_tracker(ticket)).second;
    MALIPUT_DRAKE_DEMAND(added);  // Shouldn't have been there.
  }
}

void DependencyGraph::RepairTrackerPointers(const DependencyGraph& source,
                                            const DependencyTracker::PointerMap& tracker_map,
                                            const internal::ContextMessageInterface* owning_subcontext,
                                            Cache* new_cache) {
  MALIPUT_DRAKE_DEMAND(owning_subcontext != nullptr);
  ...
  for (DependencyTicket ticket(0); ticket < trackers_size(); ++ticket) {
    if (!has_tracker(ticket)) continue;
    get_mutable_tracker(ticket).RepairTrackerPointers(source.get_tracker(ticket), tracker_map, owning_subcontext,
                                                      new_cache);
  }
}
