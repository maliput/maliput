...
class TypeSafeIndex {
 public:
  ...
  explicit TypeSafeIndex(int64_t index) : index_(static_cast<int>(index)) {
    ...
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index, "Explicitly constructing an invalid index."));
  }
  ...
  TypeSafeIndex(TypeSafeIndex&& other) noexcept : index_(other.index_) { other.index_ = kDefaultInvalid; }
  ///@}
  ...
  const TypeSafeIndex& operator++() {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-incrementing an invalid index."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertNoOverflow(1, "Pre-incrementing produced an invalid index."));
    ++index_;
    return *this;
  }
  ...
  TypeSafeIndex operator++(int) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Post-incrementing an invalid index."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertNoOverflow(1, "Post-incrementing produced an invalid index."));
    ++index_;
  ...
  /// non-negative.
  const TypeSafeIndex& operator--() {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-decrementing an invalid index."));
    --index_;
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-decrementing produced an invalid index."));
    return *this;
  }
  ...
  /// non-negative.
  TypeSafeIndex operator--(int) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Post-decrementing an invalid index."));
    --index_;
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Post-decrementing produced an invalid index."));
    return TypeSafeIndex(index_ + 1);
  }
  ///@}
  ...
  TypeSafeIndex& operator+=(int i) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with an int on an invalid index."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertNoOverflow(i, "In-place addition with an int produced an invalid index."));
    index_ += i;
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with an int produced an invalid index."));
    return *this;
  }

  /// Allow addition for indices with the same tag.
  TypeSafeIndex<Tag>& operator+=(const TypeSafeIndex<Tag>& other) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with another index invalid LHS."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(other.index_, "In-place addition with another index invalid RHS."));
    MALIPUT_DRAKE_ASSERT_VOID(
        AssertNoOverflow(other.index_, "In-place addition with another index produced an invalid index."));
    index_ += other.index_;
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with another index produced an invalid index."));
    return *this;
  }
  ...
  /// non-negative.
  TypeSafeIndex& operator-=(int i) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with an int on an invalid index."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertNoOverflow(-i, "In-place subtraction with an int produced an invalid index."));
    index_ -= i;
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with an int produced an invalid index."));
    return *this;
  }

  /// Allow subtraction for indices with the same tag.
  TypeSafeIndex<Tag>& operator-=(const TypeSafeIndex<Tag>& other) {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with another index invalid LHS."));
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(other.index_, "In-place subtraction with another index invalid RHS."));
    ...
    index_ -= other.index_;
    MALIPUT_DRAKE_ASSERT_VOID(
        AssertValid(index_, "In-place subtraction with another index produced an invalid index."));
    return *this;
  }
  ...
  bool operator==(const TypeSafeIndex<Tag>& other) const {
    ...
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(other.index_, "Testing == with invalid RHS."));
    return index_ == other.index_;
  }

  /// Allow equality test with unsigned integers.
  template <typename U>
  typename std::enable_if_t<std::is_integral_v<U> && std::is_unsigned_v<U>, bool> operator==(const U& value) const {
    MALIPUT_DRAKE_ASSERT_VOID(AssertValid(index_, "Testing == with invalid index."));
    return value <= static_cast<U>(kMaxIndex) && index_ == static_cast<int>(value);
  }
  ...
  friend void hash_append(HashAlgorithm& hasher, const TypeSafeIndex& i) noexcept {
 ...
 private:
  ...
  static void AssertValid(int64_t index, const char* source) {
    if (index < 0 || index > kMaxIndex) {
      throw std::runtime_error(std::string(source) + " Type \"" +
  ...
  void AssertNoOverflow(int delta, const char* source) const {
    if (delta > 0 && index_ > kMaxIndex - delta) {
      throw std::runtime_error(std::string(source) + " Type \"" +
                               maliput::drake::NiceTypeName::Get<TypeSafeIndex<Tag>>() + "\", has overflowed.");
    }
  }
...
typename std::enable_if_t<std::is_integral_v<U> && std::is_unsigned_v<U>, bool> operator==(
    const U& value, const TypeSafeIndex<Tag>& tag) {
