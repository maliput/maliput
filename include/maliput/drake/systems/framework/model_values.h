...
template <typename T>
void ModelValues::AddVectorModel(int index, std::unique_ptr<BasicVector<T>> model_vector) {
  if (model_vector.get() != nullptr) {
    AddModel(index, std::make_unique<Value<BasicVector<T>>>(std::move(model_vector)));
  } else {
